...
# ── HELPER FUNCTION: RENDER CANVAS & DOWNLOAD ──────────────────────────────
def show_canvas(before, after=None, title_after="", is_gray_after=False, is_gray_before=False):
    if after is None:
        ...
        with col2:
            ...
            # Konversi array numpy gambar menjadi format bytes .png
            if is_gray_after:
                is_success, buffer = cv2.imencode(".png", after)
            else:
                # Karena gambar 'after' dilempar dalam format RGB untuk UI, kita kembalikan ke BGR untuk OpenCV
                after_bgr = cv2.cvtColor(after, cv2.COLOR_RGB2BGR)
                is_success, buffer = cv2.imencode(".png", after_bgr)
                
            if is_success:
                # Membuat nama file otomatis dan rapi tanpa spasi
                clean_filename = f"VisionStudio_{title_after.replace(' ', '_').replace('(', '').replace(')', '')}.png"
...
def apply_morphology(img, operation, se_shape, se_size):
    ...
    # PERUBAHAN: Menghilangkan opsi "Silang" dari kamus shape
    shapes = {"Persegi": cv2.MORPH_RECT, "Elips": cv2.MORPH_ELLIPSE}
    se = cv2.getStructuringElement(shapes[se_shape], (se_size, se_size))
    # PERUBAHAN: Menghilangkan opsi Opening dan Closing
    ...
    if img1 is not None:
        ...
        with t6:
            ...
            with c_tools:
                ...
                # PERUBAHAN: Menghilangkan opsi Opening dan Closing dari Selectbox
                m_op = st.selectbox("Operasi", ["Erosi", "Dilasi"])
                # PERUBAHAN: Menghilangkan opsi Silang dari Selectbox
                se_shape = st.selectbox("Element (SE)", ["Persegi", "Elips"])
